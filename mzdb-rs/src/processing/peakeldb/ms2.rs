...
use crate::processing::signal::ms2_detection::{IsolationWindow, DiaMs2PeakelRecord};
use crate::processing::model::HasPeakelData;
use super::common::{chrono_lite_timestamp, ExtendedPeakel, PeakelSerializer, PeakelWriterStats, PeakelDbWriter};
...
/// use mzdb::processing::IsolationWindow;
///
/// let windows: Vec<IsolationWindow> = vec![]; // discovered beforehand
/// let mut writer = Ms2PeakelDbWriter::create("output.peakeldb", "input.mzDB", &windows).unwrap();
...
impl Ms2PeakelDbWriter {
    ...
    ///
    /// Creates the database, inserts metadata and isolation windows, and begins
    /// a transaction for peakel insertion. The transaction is committed when
    /// [`close`](PeakelDbWriter::close) is called or on [`Drop`].
    ///
    /// # Arguments
    ...
    pub fn create<P: AsRef<Path>>(path: P, mzdb_filename: &str, windows: &[IsolationWindow]) -> Result<Self> {
